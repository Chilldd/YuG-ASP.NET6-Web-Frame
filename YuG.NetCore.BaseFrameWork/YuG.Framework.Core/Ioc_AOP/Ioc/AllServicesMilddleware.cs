...
namespace YuG.Framework.Core.Ioc
{
    ...
    {
        public static void UseAllServiceMilddleware(this IApplicationBuilder app, IServiceCollection _services, IConfiguration configuration)
        {
            if (app == null) throw new ArgumentNullException(nameof(app));

            var option = configuration.GetSection(IocOption.ApplicationOptionName).Get<IocOption>();
            if (option is null)
                throw new YuGCustomizeException($"【Ioc服务查询】缺少配置信息【{IocOption.ApplicationOptionName}】");
            option.CheckModel();

            SystemServerInformationHelper.OpenMiddlewareInformation(SystemServerEnum.QueryIocType, $"访问地址：【{option.QueryAllServiceUrl}】");

            List<Type> tsDIAutofac = new List<Type>();
            tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, $"{option.ServiceDLLName}.dll")).GetTypes().ToList());
            tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, $"{option.RepositoryDLLName}.dll")).GetTypes().ToList());

            app.Map($"{option.QueryAllServiceUrl}", builder => builder.Run(async context =>
            {
